...
use std::{net::{SocketAddr}, sync::{Arc, Mutex, atomic::{AtomicBool, Ordering}}, collections::HashMap};
use crate::{HeroinnProtocol, protocol::{TUNNEL_FLAG, tcp::TcpConnection}};
use self::wrapper::{RUdpServer, RUdpClient};
...
impl Server for UDPServer{
    ...
    >(
        ...
        let connections : Arc<Mutex<HashMap<SocketAddr , Arc<RUdpClient>>>> = Arc::new(Mutex::new(HashMap::new()));
        ...
        std::thread::Builder::new().name(format!("udp main worker : {}" , local_addr_1.clone())).spawn(move || {
            ...
            loop{
                ...
                connections_1.lock().unwrap().insert(remote_addr.clone(), client.clone());

                std::thread::Builder::new().name(format!("udp client worker : {}" , local_addr_1.clone())).spawn(move || {
                    loop{
                        ...
                        if buf.len() == TUNNEL_FLAG.len() + 3{
                            if buf[1..5] == TUNNEL_FLAG{
                                ...
                                std::thread::Builder::new().name(format!("udp tunnel worker1 : {}" , tunnel_client.local_addr().unwrap())).spawn(move || {
                                    loop{
                                ...
                                std::thread::Builder::new().name(format!("udp tunnel worker2 : {}" , tunnel_client.local_addr().unwrap())).spawn(move || {
                                    ...
                                    loop{
                                        ...
                                        if let Err(e) = tunnel_client_2.send(&mut buf[1..]){
                                            log::error!("udp sender error : {}" , e);
                                            break;
                                        }
                                    }
                        ...
                        cb_data.lock().unwrap()(HeroinnProtocol::UDP , buf[1..].to_vec(), remote_addr, cbcb);
                    }
        ...
        match self.connections.lock().unwrap().get_mut(&peer_addr){
            Some(k) => {
                ...
                match k.send(data){
                    Ok(_) => {},
                    Err(e) => {
                        return Err(std::io::Error::new(std::io::ErrorKind::Interrupted , format!("udp send msg error : {}", e)));
                    },
                };
                Ok(())
            },
...
impl Client for UDPConnection{
    ...
    fn recv(&mut self) -> std::io::Result<Vec<u8>> {
        ...
        match s.recv(){
            ...
            Err(e) => {
                return Err(std::io::Error::new(std::io::ErrorKind::Interrupted , format!("udp receive error : {}", e)));
            },
        }
    }
...
impl Clone for UDPConnection{
    fn clone(&self) -> Self {
        Self { s : self.s.clone(), closed : self.closed.clone() , local_addr : self.local_addr.clone() }
    }
}
