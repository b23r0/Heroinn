use heroinn_util::{rpc::RpcMessage, ftp::{method::{get_disk_info, get_folder_info, remove_file, md5_file}, FileInfo, FTPPacket, FTPId, FTPGetHeader, FTPPutHeader}, protocol::{tcp::TcpConnection, Client}, packet::TunnelRequest, cur_timestamp_secs};
...
pub fn download_file(sender : &Sender<FTPPacket> , local_path : &String , remote_path : &String) -> Result<()>{
    ...
    std::thread::Builder::new().name("download file worker".to_string()).spawn(move || {
        ...
        // init transfer log
        {
            ...
            transfer.insert(local_path.clone() ,TransferInfo{
...
pub fn upload_file(sender : &Sender<FTPPacket> , local_path : &String , remote_path : &String)  -> Result<()> {
    ...
    match G_RPCCLIENT.wait_msg(&msg.id, 10){
        Ok(p) => {
            ...
            if p.retcode == 0{
                let (remote_file_md5, remote_file_size) = (p.data[0].clone() , p.data[1].parse::<u64>().unwrap());
    ...
    let local_path = local_path.clone();
    let remote_path = remote_path.clone();
    std::thread::Builder::new().name("upload file worker".to_string()).spawn(move || {
        let server = TcpListener::bind("127.0.0.1:0").unwrap();
